...
impl ProcessOutgoingRecordImpl for OutgoingCreditCardsImpl {
    ...
    fn fetch_outgoing_records(
        ...
        let payload_from_data_row: &dyn Fn(&Row<'_>) -> Result<Payload> =
            &|row| Ok(InternalCreditCard::from_row(row)?.to_payload().unwrap());
        ...
        common_save_outgoing_records(&tx, "credit_cards_sync_outgoing_staging", staging_records)?;
    ...
    fn push_synced_items(
        ...
        let outgoing_table_name = "credit_cards_sync_outgoing_staging";
        let data_table_name = "credit_cards_data";
        ...
        common_push_outgoing_records(&tx, "credit_cards_mirror", outgoing_table_name)?;
